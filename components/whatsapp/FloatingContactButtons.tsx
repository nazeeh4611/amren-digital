...
const buttonBase = "flex h-14 w-14 items-center justify-center rounded-full shadow-[var(--shadow-card)]";

/**
 * Persistent, low-key call + WhatsApp entry points — two fixed buttons
 * stacked in the corner, not a popup, so they stay available on every
 * page without interrupting reading. Sit higher while the cookie consent
 * banner is undecided so the two never overlap it, then settle to the
 * corner once it's dismissed.
 */
export function FloatingContactButtons() {
  ...
  return (
    <div
      className={clsx(
        "fixed right-5 z-40 flex flex-col-reverse items-center gap-3 transition-[bottom] duration-300 sm:right-7",
    ...
    >
      <motion.a
        ...
        className={clsx(buttonBase, "bg-[#25D366] text-white")}
      >
        <WhatsAppIcon />
      </motion.a>
      ...
      <motion.a
        ...
        onClick={() => trackEvent("phone_click", { label: "floating_button" })}
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.4, delay: 1.15, ease: [0.16, 1, 0.3, 1] }}
        whileHover={{ scale: 1.06 }}
        whileTap={{ scale: 0.96 }}
        aria-label={`Call AMREN Digital at ${site.contact.phone2}`}
        className={clsx(buttonBase, "bg-navy text-cream")}
      >
        <PhoneIcon />
      </motion.a>
    </div>
